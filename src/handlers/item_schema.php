...
function ensure_items_status_column($conn) {
    if (items_table_has_column($conn, 'status')) {
        if (!db_execute_statement($conn, "ALTER TABLE items MODIFY status varchar(50) NOT NULL DEFAULT ''")) {
...
function ensure_items_quantity_column($conn) {
    ...
    if (!db_execute_statement($conn, "ALTER TABLE items MODIFY quantity int(11) NOT NULL DEFAULT 0")) {
...
function drop_items_capital_column($conn) {
    ...
    if (!db_execute_statement($conn, "ALTER TABLE items DROP COLUMN capital")) {
