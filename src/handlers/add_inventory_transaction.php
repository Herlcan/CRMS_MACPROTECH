...
if ($capital < 0 || $stock_in <= 0 || $markup_percentage < 0) {
    redirectStockRecordsWithDialog($item_id, 'error', 'Stock Not Added', 'Capital, stock-in, and markup must be valid non-negative values.');
}
...
try {
    ...
    sync_item_from_inventory_transactions($conn, $item_id, $markup_percentage);
